...
parser.add_argument('--test_appendix', type=str, default='1')
...
def main():
    ...
    checkpoint_path = f"{args.checkpoint}/{args.experiment_name}/{args.dataset}/" \
                      f"{args.arch}_D{args.depth}_W{args.widen_factor}/" \
    ...
    if args.resume and not args.evaluate:
        ...
        try:
            ...
            if os.path.isfile(f'{checkpoint_path}/checkpoint.pth.tar'):
                ...
                for attack_method_s in list_attack_methods:
                    for nl in noise_levels[attack_method_s]:
                        names += [f'{attack_method_s}_rtest_loss-{nl}', f'{attack_method_s}_rtest_acc-{nl}',
                                  f'{attack_method_s}_rtest_acc_topk-{nl}']
        ...
        try:
            ...
            print(
                f'==> Resume dir incorrect.. '
                f'loading from default checkpoint_path: {checkpoint_path}')
        checkpoint = torch.load(f'{checkpoint_path}/model_best.pth.tar',
                                map_location=DEVICE)
        best_acc = checkpoint['best_acc']
        start_epoch = checkpoint['epoch']
        model.load_state_dict(checkpoint['state_dict'])
        ...
        for attack_method_s in list_attack_methods:
            attack_method = eval(attack_method_s)
            for nl in noise_levels[attack_method_s]:
                ...
                names += [f'{attack_method_s}_rtest_loss-{nl}',
                          f'{attack_method_s}_rtest_acc-{nl}',
                          f'{attack_method_s}_rtest_acc_topk-{nl}']
                print(f'Robustness test {nl}'+ ': Loss:  %.8f, Acc:  %.2f, Acc Topk:  %.2f' % (
                rtest_loss, rtest_acc, rtest_acc_topk))
        with open(f'{checkpoint_path}/test_result_{args.test_appendix}.csv', 'w') as f:
            write = csv.writer(f)
            write.writerow(names)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for attack_method_s in list_attack_methods:
            ...
            for nl in noise_levels[attack_method_s]:
                ...
                }, False, checkpoint=checkpoint_path,
